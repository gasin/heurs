        ...
        use_effect_with((), move |_| {
            if let Some(canvas) = canvas_ref.cast::<web_sys::HtmlCanvasElement>() {
                ...
                for i in 0..num_x {
                    for j in 0..num_y {
                        if (i + j) % 2 == 0 {
                            context.set_fill_style(&color_light.into());
                        } else {
                            context.set_fill_style(&color_dark.into());
                        }
                        context.fill_rect(
                ...
                context
                    ...
                    .unwrap();
                context.set_fill_style(&"rgba(25, 135, 84, 0.8)".into());
                context.fill();
                context.set_stroke_style(&"#0f5132".into());
